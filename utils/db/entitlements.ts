...
import { entitlementsTable, coursesTable, usersTable } from "@/utils/db/schema";
...
export async function grantUserEntitlement(stripeUserId: string, stripeProductId: string, stripeSessionId: string) {
    const productId = await db.select({ id: coursesTable.id }).from(coursesTable).where(eq(coursesTable.stripeProductId, stripeProductId));
...
export async function getUserEntitlements(userId: string) {
    return db.select()
        ...
        .leftJoin(coursesTable, eq(entitlementsTable.courseId, coursesTable.id))
